...
export async function PUT(request: Request, { params }: { params: { id: string } }) {
  const { id } = params;
  const { expenditures,description,date,amount,document } = await request.json();
  
  
  await connectDB();
  const updatedExpenditure = await Expenditure.findByIdAndUpdate(id, {
    expenditures,description,date,amount,document
  }, { new: true });

  

  let response = NextResponse.json({ message: "Expenditure updated" }, { status: 200 });
  setCORSHeaders(response);
  return response;
}
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (!deletedExpenditure) {
      let response = NextResponse.json({ message: "Expenditure not found" }, { status: 404 });
      setCORSHeaders(response);
      return response;
    }
    ...
    let response = NextResponse.json({ message: "Expenditure deleted" }, { status: 200 });
    setCORSHeaders(response);
    return response;
  } catch (error) {
